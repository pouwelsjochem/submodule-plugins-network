...
sed -e '/%LUA_HELPER_CODE%/r /tmp/tmpfile1.txt' -e 's/%LUA_HELPER_CODE%//' <src/network/LuaHelper.java.template >src/network/LuaHelper.java
rm -f /tmp/tmpfile1.txt
...
if [ -z "$OUTPUT_DIR" ]
then
    OUTPUT_DIR=$path/../../build-core/$PLUGIN_NAME/android
fi
...
then
	...
	zip -d "$path/bin/${PLUGIN_NAME}.jar" "com/*" "*.orig"

	# Remove the LuaHelper.java.template file
	zip -d "$path/bin/${PLUGIN_NAME}.jar" "network/LuaHelper.java.template"

	echo "SUCCESS: Plugin binary is located at: '$path/bin/${PLUGIN_NAME}.jar'"

	cp -v "$path/bin/${PLUGIN_NAME}.jar" "$path/../../build-core/${PLUGIN_NAME}/android/."
else
